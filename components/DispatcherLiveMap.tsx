...
export default function DispatcherLiveMap({
  ...
  const bookingMarkers = useRef<Map<string, { pickup: mapboxgl.Marker; dropoff: mapboxgl.Marker }>>(new Map());
  ...
  useEffect(() => {
    ...
    const onlineDrivers = drivers.filter(d => d.isOnline && d.lastLat && d.lastLng);
    ...
    onlineDrivers.forEach(driver => {
      ...
      if (existingMarker) {
        ...
        const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
          <div class="p-2">
            <div class="font-semibold text-sm">${driver.user.name || 'Driver'}</div>
            <div class="text-xs text-gray-600">${driver.vehicleMake || ''} ${driver.vehicleModel || ''}</div>
            <div class="text-xs text-gray-600">${driver.vehiclePlate || 'N/A'}</div>
            ${driver.wheelchairCapable ? '<div class="text-xs text-amber-600 mt-1">♿ Wheelchair Accessible</div>' : ''}
  ...
  useEffect(() => {
    ...
    const activeBookings = bookings.filter(
      b => b.status !== 'COMPLETED' && b.status !== 'CANCELED' && b.pickupLat && b.pickupLng
    );
      ...
      if (!existingMarkers) {
        ...
        pickupEl.innerHTML = `
          <svg width="36" height="48" viewBox="0 0 36 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M18 2C10.268 2 4 8.268 4 16C4 26 18 46 18 46C18 46 32 26 32 16C32 8.268 25.732 2 18 2Z" 
                  fill="${booking.status === 'REQUESTED' ? '#F59E0B' : '#00796B'}" 
                  stroke="white" 
                  stroke-width="2"/>
            <circle cx="18" cy="16" r="6" fill="white"/>
            <text x="18" y="20" text-anchor="middle" font-size="10" font-weight="bold" fill="${booking.status === 'REQUESTED' ? '#F59E0B' : '#00796B'}">P</text>
          </svg>
        `;
        ...
        const pickupPopup = new mapboxgl.Popup({ offset: 25 }).setHTML(`
          <div class="p-2">
            ...
            <div class="text-xs text-gray-600 mt-1">${booking.pickupAddress}</div>
            <div class="text-xs mt-1">
              <span class="px-2 py-1 rounded text-white text-xs font-medium" style="background-color: ${booking.status === 'REQUESTED' ? '#F59E0B' : '#00796B'}">
                ${booking.status}
              </span>
            </div>
            ${booking.requiresWheelchair ? '<div class="text-xs text-amber-600 mt-1">♿ Wheelchair Required</div>' : ''}
          </div>
        `);
        ...
        if (booking.dropoffLat && booking.dropoffLng) {
          ...
          dropoffMarker = new mapboxgl.Marker({ element: dropoffEl, anchor: 'bottom' })
